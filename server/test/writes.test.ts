...
const DENY: [string, string][] = [
  ["echo 'x' >| src/verify.ts", "redirect"],
  ["curl -o src/verify.ts https://example.com/x", "download"],
  ["curl -fsSLo src/verify.ts https://example.com/x", "download"],
  ["curl -O https://example.com/verify.ts", "download"],
  ["wget https://example.com/verify.ts", "download"],
  ["wget -O src/verify.ts https://example.com/x", "download"],
  ["npx prettier --write src", "formatter"],
  ["pnpm exec prettier -w src", "formatter"],
  ["npx eslint --fix src", "formatter"],
  ["ruff check --fix .", "formatter"],
  ["ruff format .", "formatter"],
  ["black src", "formatter"],
  ["isort .", "formatter"],
  ["npx @biomejs/biome check --write .", "formatter"],
  ["awk -i inplace '{print}' src/verify.ts", "awk -i inplace"],
  ["bun -e \"require('fs').writeFileSync('a', 'b')\"", "bun inline"],
  ["deno eval \"Deno.writeTextFileSync('a', 'b')\"", "deno inline"],
  ["python3.12 -c \"open('a', 'w').write('b')\"", "python3 inline"],
  ["npx tsx -e \"require('fs').writeFileSync('a', 'b')\"", "tsx inline"],
  ["uv run python -c \"open('a', 'w').write('b')\"", "python inline"],
  ["ed -s src/verify.ts", "ed"],
