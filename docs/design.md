...
The `commit` on an unaided entry is HEAD when the witness ran, which may be dirty. Version one records it and does not verify it. The CI policy gate, later, re-runs the witness command at that commit.
...
- `last` is the previous step's closing result, kept for one prompt so the prompt hook can show it.
...
- **`belay_hint(skill, text)`** Only on a `you` step. Increments `hints`. Returns `{rung, of: 3, kind}` where kind is `concept`, `repo`, or `pseudocode`, plus the skill's `precedents` on rung two. On a fourth call returns `{rung: 4, kind: "escalate"}`: Claude writes the senior a ten-minute question with what was tried, and gives no further hints. The hint's text, passed as `text`, is journaled and never written to the logbook.
...
- **`belay_init(map, precedents?)`** Sets a repo up. `map` is `typescript` or `python`, naming a starter map shipped inside the server bundle. Writes `.belay/map.json` from it, with `precedents` merged in per skill id when given; writes `"outputStyle": "belay:Belay"` into `.claude/settings.json`, creating the file or merging into it and keeping every other key; adds `.belay/state.json` to `.gitignore` if it is not there. Refuses to overwrite an existing map and says so. Returns the paths it wrote.
...
**`session-start`**
Reads: whether `.belay/map.json` exists.
Writes: nothing.
...
Reads: `state.json`.
Writes: nothing.
Returns: nothing when there is no open step or the mode is `review` or `quiet`. On a `you` step, for the edit tools, denies with reason "`<skill>` is unearned. You write it. Want a hint?" For Bash on a `you` step, denies when the command matches a write pattern and allows otherwise. Write patterns, version one: `>` or `>>` not followed by `&` and not targeting `/dev/null`; `tee`; `sed -i`; `perl -i`; `cp`, `mv`, `rm`, `mkdir`, `touch`, `ln`; `git apply`, `git checkout --`, `git restore`, `git stash pop`, `git reset --hard`, `patch`; `npx create-`, `npm init`, `npm install` and equivalents, including `pip install`, `uv add`, `uv pip install`, `poetry add`, `pipx install`; heredocs. Read-only git, test, build, type-check, `ls`, `cat`, `grep`, `find` are allowed. A denied Bash command's reason names the pattern that matched.
...
**`witness`** (PostToolUse on Bash)
Reads: `state.json`, the command, the tool response.
...
Then it recognizes a witness by the command: `vitest`, `jest`, `node --test`, `npm test`, `pnpm test`, `bun test`, `pytest`, `python -m pytest`, `python -m unittest`, `uv run pytest` are `test`; `tsc`, `mypy`, `pyright` are `types`; `npm run build`, `pnpm build`, `bun run build`, `vite build`, `next build`, `esbuild` are `build`. Pass is exit code 0 when the tool response carries one, otherwise parsed from the runner's summary line, and unknown counts as a fail. Anything else is not a witness and the hook returns nothing.
Writes: appends the witness to `step.witnesses`. Then, on a `you` step with a passing witness of an accepted kind, with `toolEdits` empty, and with a non-empty diff against `baseline` (tracked changes plus untracked files, excluding `.belay/`), sets `step.pending` to the witness, HEAD, and the files. It writes nothing to the logbook.
...
6. Person answers. Claude calls `belay_answer(correct: true)`: the `unaided` entry is written with `hints: 1` and the question, and the step closes. Claude relays the count and tells them to make the change.
...
A plugin's output style is namespaced by the plugin name, so the setting that turns the contract on is `"outputStyle": "belay:Belay"`. A plugin cannot force its style on. The repo's committed `.claude/settings.json` carries the setting, so the contract is ambient for everyone who opens the repo, and `/belay:learn` and `/belay:team` write it. Without the style, Claude still calls the tools but explains the whole solution on an unearned step, which the first acceptance run showed.
